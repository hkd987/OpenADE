...
- **Team collaboration is asynchronous; live multi-user is out of
  scope.** The open, supported team surface is the self-hosted workspace
  hub (`crates/openade-server` + the daemon's share/pickup integration —
  see [docs/multiplayer.md](docs/multiplayer.md)): manual per-session
  sharing, browsing, and cross-harness pickup, plus the Git-native
  surfaces (review branches, the shared memory repo). *Live* shared
  sessions — real-time observation, co-driving, presence, shared
  daemons — are intentionally not implemented here and contributions
  adding them will not be accepted. Each member's daemon stays
