...
| 50 | Signal ingestion (release binaries) | Release `openade-server` + release daemon connected via env: a Sentry-style `POST /signals` with evidence, join keys, and `affected_count: 10` answered `{inserted: 1}`; the daemon proxy (`GET /inbox?status=new`) lists the item with severity `critical` and the affected rollup — `config` reports `inbox_backend: remote` |
| 51 | Dismissal + escalation | `POST /inbox/1/dismiss {intended_behavior}` stamped `decided_by: casey` (from the member token) and snapshotted the impact; reposting the same fingerprint with `affected_count: 30` (3×) answered `{escalated: 1}` — the item returned to `new` with an escalation note appended, and the dismissal stayed queryable in outcome memory |
| 52 | Triage session | `POST /sessions/from-inbox` launched `triage: manual NPE` (claude shim) with `inbox_item_id` set; the worktree's `.openade/inbox-item.md` carries the sentry evidence link, `v2.3.0` join key, and the "Prior outcomes on this fingerprint" history incl. the dismissal; the item shows "taken by casey" to every member |
| 53 | Outcome memory via gh | With a `gh` shim reporting the branch PR MERGED: `POST /sessions/{id}/inbox-outcome` recorded `{kind: merged, pr_url, recorded: true}`; the second call answered `recorded: false` (idempotent); the item's history reads `[merged (PR), dismissed]` — newest first, anchored to the fingerprint |
| 54 | Local inbox fallback | A second release daemon with NO server env reports `inbox_backend: local`; `POST /signals` at the daemon (no auth, loopback) landed `solo flake` in its embedded inbox — the same surface with zero configuration and no login anywhere |
